  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 text-sm">
      <div className="flex flex-wrap items-center gap-3">
        <Badge className={routingDecision.mode === 'adaptive' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}>
          ...
          {sortedChannels.map(([channel, probability]) => {
            ...
            return (
              <div key={channel} className="flex items-center gap-2">
                <span className={`w-20 text-xs truncate ${isSelected ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>
                  {isSelected && <CheckCircle className="inline h-3 w-3 mr-1 text-green-600" />}
                  {channel}
                </span>
                <div className="flex-1 h-4 bg-gray-100 rounded overflow-hidden">
                  <div
                    className={`h-full rounded ${isSelected ? 'bg-blue-500' : 'bg-gray-300'}`}
                    style={{ width: `${widthPercent}%` }}
                  />
                </div>
                <span className={`w-14 text-right text-xs tabular-nums ${isSelected ? 'font-semibold text-gray-900' : 'text-gray-500'}`}>
      ...
      <div className="mt-3 rounded bg-gray-50 p-2.5 text-xs text-gray-600 leading-relaxed">
